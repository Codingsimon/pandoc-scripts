...
find . -maxdepth ${SEARCH_DEPTH} -name 'book*.sh' -print0 | xargs -0 -I{} -n1 -P1 /bin/bash -c './bin/makepdf-book-sh.sh "{}"'
fi

if [[ ${CREATE_MANUAL_BOOKS} = true ]] ; then
find . -maxdepth ${SEARCH_DEPTH} -name 'book*.txt' -print0 | xargs -0 -I{} -n1 -P8 /bin/bash -c './bin/makepdf-book-sh.sh "{}"'
fi
